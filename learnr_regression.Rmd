...
question("Which of the following statements is true?",
         answer("growth rate is the independent variable and tannin is dependent", correct = TRUE),
         answer("the response is the growth rate and tannin is explanatory"),
...
amount of tannins in the diet (x-axis). You can use either the `plot()` function
or the `gf_point()` function. The `gf_` or "graph formula" functions are more
flexible than base graphics, but either can be used here.
...
The first column gives the names of the parameters estimated by your linear
model. Here it is two parameters:
...
The second column, headed `Estimate` gives the values calculated by the linear
model for your two parameters:

* `r round(caterpiller_lm$coefficients[1],2)` for the intercept
* `r round(caterpiller_lm$coefficients[2],2)` for the gradient. This is negative as growth rate is low with high tannin

The third column `Std. Error` provides the standard errors for your estimates. 

The fourth column `t value` gives the estimated statistic from the **t distribution**
which is a statistical distribution (developed to improve the brewing of Guiness stout!).

The fifth and final column, headed `Pr(>|t|)` indicates how likely you are to have
obtained these values from a t distribution according to the null hypothesis. That's
very confusing, a simpler interpretation is "what is the probability of obtaining
these estimated intercepts and gradient if there was no relationship between 
caterpiller growth rate and tannin". Basically, the **lower** these **p-values**
the **more important** the gradient and intercept.
...
```{r understanding_summary, echo=FALSE, allow_retry=TRUE}
...
Still to add. QQ plots, residuals etc.
...
you should give the *degrees of freedom (see other tutorial)* as well as the F-statistic.